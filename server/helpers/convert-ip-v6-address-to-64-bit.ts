...
export const convertIpV6AddressTo64Bit = (ipV6Address: string): string => {
  ...
  if(fullAddress.includes('::')) {  // eslint-disable-line neos-eslint-plugin/comment-colon-spacing
    const parts = ipV6Address.split('::');  // eslint-disable-line neos-eslint-plugin/comment-colon-spacing
  ...
  const prefix64 = `${fullAddress.split(':').slice(0, 4).join(':')}::/64`;  // eslint-disable-line neos-eslint-plugin/comment-colon-spacing
  return prefix64;
};
