...
adminLogin.post('/', async context => {
  if(isEmpty(context.env.ADMIN_PASSWORD) || isEmpty(context.env.ADMIN_JWT_SECRET)) return context.json({ error: 'エラーが発生しました' }, 500);
  ...
  if(parsed.password !== context.env.ADMIN_PASSWORD) return context.json({ error: `${adminPasswordDisplayName}が一致しません` }, 401);
  ...
  return context.json({ result: { token } }, 200);
});
