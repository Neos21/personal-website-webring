...
export function DeleteSiteForm({ site }: Props): ReactElement {
  ...
  const onSubmit = async (event: SubmitEvent<HTMLFormElement>): Promise<void> => {
    ...
    const parsedResult = deleteSiteSchema.safeParse(payload);
    if(!parsedResult.success) return setClientError(mergeIssues(parsedResult.error));
    
    setIsSubmitting(true);
    try {
      await ky.delete(`/api/sites/${site.id}`, { json: parsedResult.data }).json();
      alert('サイトを削除しました。');
      navigate('/list');
    }
    catch(error) {
  ...
  return (
    <form onSubmit={onSubmit} style={{ marginTop: '3rem', padding: '1rem', border: '1px solid #ffcccc', borderRadius: '8px', background: '#fffafa' }}>
      <fieldset style={{ border: 'none', padding: 0, margin: 0 }}>
        ...
        <p className="form-label-memo">サイトを論理削除します。実行するには管理パスワードを入力してください。</p>
