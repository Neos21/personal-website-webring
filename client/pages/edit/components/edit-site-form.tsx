...
export function EditSiteForm({ site }: Props): ReactElement {
  ...
  const [isShownBanner, setIsShownBanner] = useState<boolean>(!isEmpty(site.banner_url) && isImageUrl(site.banner_url));
  ...
  return (
    <form className="mb-8 space-y-6" onSubmit={onSubmit}>
      ...
      <fieldset>
        ...
        <div className="text-muted text-sm">{site.is_self === 1 ? '更新を適用するために管理パスワードを入力してください。' : '自薦に切り替えるため、新しいパスワードを設定してください。'}</div>
