...
export default function Next(): ReactElement {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const response = await ky.get(`/api/next${id != null ? '?id=' + id : ''}`).json<{ result: SiteNameUrl; }>();
        console.log('Next', response.result);
        location.href = response.result.url;  // 遷移開始
        setSite(response.result);
      }
      catch(error) {
        console.error(error);
        setError(extractApiErrorMessage(error, '次のサイトにジャンプできませんでした'));
      }
      finally {
        setIsLoading(false);
      }
  ...
  return (
    <main>
      ...
      {!isLoading && !isEmpty(error) && (
        <>
          <h1 title="次のサイトにジャンプ">{appConstants.siteNameJapanese}</h1>
